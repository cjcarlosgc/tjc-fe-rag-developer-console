...
**Estado:** aprobado para trazas experimentales; la extensión a contexto completo de `AnalysisRun` sigue propuesta, sin contrato live.
**Historias:** HU17 (traza experimental) y HU15 (extensión propuesta, no aceptada).

## Objetivo

Mostrar en una pantalla amplia y navegable la evidencia de contexto recolectada para una comparación experimental, con dos modos que comparten shell y panel lateral pero mantienen semánticas distintas: `Contexto RAG` y `Exploración del agente`. El contexto completo del `AnalysisRun` operativo requiere un contrato adicional.

## Entradas

- Desde un experimento: abrir una repetición concreta `RAG` o `GENERALIST_AGENT` sobre el `AnalysisRun` de origen.
...
- El panel lateral muestra herramienta/paso, argumentos, archivo/símbolo, rango, snippet, dos líneas circundantes (mismo componente y misma regla de truncamiento que HU17), truncamiento y hash.
...
HU17) y trayectoria del agente (HU17) — nunca conocimiento funcional
