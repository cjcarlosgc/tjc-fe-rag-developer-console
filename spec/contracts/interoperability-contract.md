...
- `GET /analysis-runs?status&cursor&limit` -> `200 Page<AnalysisRunSummaryResponse>` (HU55, sin `projectId`: Runs de todos los Projects visibles para el usuario autenticado (cualquier rol), mismo shape — no es una vista global sin dueño; cubre los Projects personales del usuario más los de organización con registro de acceso ya existente). **Implementado (HU55, bundle B).**
...
Fuente: `DEC-ORG-001` (HU58-HU64). GitHub es la fuente de verdad de la autorización; este contrato no repite las decisiones de producto, define su superficie HTTP. **Implementado (bundle A y bundle B).**
