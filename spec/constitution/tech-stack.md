...
`VITE_AUTH_MODE=mock|supabase` (opcional: sin definir se deriva de `VITE_DATA_SOURCE`, `live` → `supabase`; a su vez `VITE_DATA_SOURCE` sin definir es `mock` en `vite dev` y `live` en builds de producción) separa la identidad local demostrativa de la autenticación real. `mock` solo es válido con datos mock/locales y nunca fabrica un Bearer para un Core live. La identidad GitHub se resuelve mediante Supabase Auth; la GitHub App y sus credenciales pertenecen a Core.
