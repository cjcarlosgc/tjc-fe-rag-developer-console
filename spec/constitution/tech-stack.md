...
Tampoco recibe `SANDBOX_SERVICE_TOKEN`: ese Bearer es un detalle privado Core↔Sandbox. Para los POST idempotentes de `INTEROP-2.0`, el navegador genera UUIDs con una API estándar (`crypto.randomUUID()` o equivalente compatible), conserva cada key mientras la acción lógica esté pendiente y la descarta al iniciar una nueva acción intencional.
