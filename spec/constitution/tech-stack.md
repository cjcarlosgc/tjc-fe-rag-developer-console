...
`VITE_AUTH_MODE=mock|supabase` separa la identidad local demostrativa de la autenticación real. `mock` solo es válido con datos mock/locales y nunca fabrica un Bearer para un Core live. La integración GitHub real no agrega dependencias mientras `DEC-GH-001` permanezca pendiente.

Tampoco recibe `SANDBOX_SERVICE_TOKEN`: ese Bearer es un detalle privado Core↔Sandbox. Para los POST idempotentes de `INTEROP-1.6`, el navegador genera UUIDs con una API estándar (`crypto.randomUUID()` o equivalente compatible), conserva cada key mientras la acción lógica esté pendiente y la descarta al iniciar una nueva acción intencional.
