...
export function AnalysisProgress({ projectVersionId, initialPollAfterMs }: Props) {
  ...
  if (!operation) return <div className="analysis-progress" role="status"><span className="spinner" /> Conectando con ProjectVersion…</div>
  ...
  return <div className="operation-stack"><div className="analysis-progress" aria-live="polite"><div className="progress-header"><div><span className="live-indicator" /><strong>{stageLabels[operation.status]}</strong><p>ProjectVersion <code>{projectVersionId}</code></p></div><span className="progress-value">{operation.status === 'COMPLETED' ? 'READY' : 'LIVE'}</span></div><ol className="stage-rail" aria-label="Etapas de indexación">{stageOrder.map((stage, index) => <li className={index < currentIndex ? 'done' : index === currentIndex ? 'active' : ''} key={stage}><span />{stageLabels[stage]}</li>)}</ol></div>{operation.status === 'COMPLETED' && resultQuery.isPending && <div className="feedback" role="status"><span className="spinner" />Cargando resumen del análisis…</div>}{operation.status === 'COMPLETED' && resultQuery.isError && <div className="structured-error" role="alert"><strong>No se pudo cargar el resumen</strong><p>{analysisErrorMessage(resultQuery.error)}</p><button className="button secondary" onClick={() => void resultQuery.refetch()}>Reintentar</button></div>}{resultQuery.data && <div className="result-summary"><div className="result-title"><h3>Resumen de ProjectVersion</h3><span className="status-badge">{resultQuery.data.status}</span></div><dl className="metric-grid"><div><dt>Framework</dt><dd>{resultQuery.data.detectedFramework ?? 'No detectado'}</dd></div><div><dt>Archivos</dt><dd>{resultQuery.data.filesProcessed}</dd></div><div><dt>Chunks</dt><dd>{resultQuery.data.chunksCount}</dd></div><div><dt>Targets</dt><dd>{resultQuery.data.targetsTotal}</dd></div><div><dt>Con test</dt><dd>{resultQuery.data.targetsWithTest}</dd></div><div><dt>Sin test</dt><dd>{resultQuery.data.targetsMissingTest}</dd></div></dl>{resultQuery.data.completedAt && <p className="completion-time">Completado: {new Date(resultQuery.data.completedAt).toLocaleString('es-PE')}</p>}</div>}</div>
}
