...
export function InventoryPage() {
  ...
  return <section><Breadcrumbs items={requestedVersionId
    ? [{ label: 'Proyectos', to: '/' }, { label: project.name, to: `/projects/${projectId}` }, { label: 'Historial de análisis', to: `/projects/${projectId}/analyses` }, { label: 'Inventario' }]
    : [{ label: 'Proyectos', to: '/' }, { label: project.name, to: `/projects/${projectId}` }, { label: 'Inventario' }]} /><div className="page-heading"><div><p className="eyebrow">Cobertura existente · {projectVersionId ?? 'sin versión'}</p><h1>Inventario testable</h1><p>Targets detectados por RAG Core, organizados por estado de cobertura.</p></div>{historical && <span className="demo-stamp">SNAPSHOT HISTÓRICO</span>}</div>{historical && <div className="contract-note history-readonly"><span className="contract-glyph">READ</span><div><strong>Inventario de sólo lectura</strong><p>Esta ProjectVersion permanece disponible como evidencia; el inventario es de consulta.</p></div></div>}{!projectVersionId ? <div className="empty-inline"><strong>Este proyecto todavía no tiene un snapshot disponible</strong><p>El historial comienza cuando un PR vinculado produce un análisis.</p></div> : inventoryQuery.isPending ? <LoadingState label="Cargando inventario…" /> : inventoryQuery.isError ? <ErrorState message={inventoryQuery.error.message} onRetry={() => void inventoryQuery.refetch()} /> : <><div className="result-summary"><div className="result-title"><h3>Resumen de cobertura</h3><span className="status-badge">{inventoryQuery.data.detectedFramework ?? 'FRAMEWORK NO DETECTADO'}</span></div><dl className="metric-grid"><div><dt>Targets</dt><dd>{inventoryQuery.data.targetsTotal}</dd></div><div><dt>Con test</dt><dd>{inventoryQuery.data.targetsWithTest}</dd></div><div><dt>Sin test</dt><dd>{inventoryQuery.data.targetsMissingTest}</dd></div></dl></div><div className="panel inventory-panel"><InventoryView targets={toInventoryTargets(inventoryQuery.data)} /></div></>}</section>
}
