...
export function RunPage() {
  ...
  return <section><Breadcrumbs items={[{ label: 'Proyectos', to: '/' }, { label: projectQuery.data?.name ?? projectId, to: `/projects/${projectId}` }, { label: 'Historial de generaciones', to: `/projects/${projectId}/runs` }, { label: `Run ${runId}` }]} /><div className="page-heading"><div><p className="eyebrow">Generation run</p><h1>Run {runId}</h1><p>Progreso y validación técnica por target.</p></div>{isMockDataSource() && <span className="demo-stamp">LIVE SIMULATION</span>}</div><RunProgress run={run} />{terminal && <><div className="flow-connector" aria-hidden="true"><span>RESULT</span><i /></div>{run.failureMessage && <div className="structured-error" role="alert"><strong>El run falló antes de procesar targets</strong><p>{run.failureMessage}</p></div>}{retryError && <div className="structured-error" role="alert"><strong>No se pudo reintentar el target</strong><p>{retryError}</p></div>}<ValidationResults run={run} onRetry={(targetId) => retryMutation.mutate(targetId)} retryingTargetId={retryMutation.isPending ? retryMutation.variables ?? null : null} /><div className="run-actions"><Link className="button primary button-link" to={`/projects/${projectId}/runs/${runId}/artifacts`}>Revisar artifacts <span aria-hidden="true">→</span></Link></div></>}</section>
}
