...
export function ExperimentPage() {
  ...
  return <section><Breadcrumbs items={[{ label: 'Proyectos', to: '/' }, { label: projectQuery.data.name, to: `/projects/${projectId}` }, { label: 'Modo experimental' }]} /><div className="experimental-heading"><div><span className="experimental-label">Modo experimental</span><h1>RAG <i>vs</i> Agente generalista</h1><p>Compara la recuperación RAG con un agente que explora el código y reúne sus propias referencias.</p></div><div className="repeat-stamp"><strong>3×</strong><span>repeticiones<br />por estrategia</span></div></div><CaptureNextPrPanel projectId={projectId} /><div className="lab-boundary"><div><p className="eyebrow">Única capacidad V1</p><h2>Comparar RAG vs Agente generalista</h2><p>Selecciona un target. Autorepair está excluido para ambas estrategias.</p><div className="field experiment-field"><label htmlFor="experiment-target">Target experimental</label><select id="experiment-target" value={selectedTarget?.id ?? ''} onChange={(event) => setTargetId(event.target.value)}>{targets.map((target) => <option value={target.id} key={target.id}>{target.methodName ?? target.symbolName} · {target.kind}</option>)}</select></div><button className="button primary" disabled={!selectedTarget || startMutation.isPending || Boolean(experimentId)} onClick={() => startMutation.mutate()}>{startMutation.isPending ? 'Creando comparación…' : 'Ejecutar comparación'}</button></div><div className={`contract-note ${isMockDataSource() ? 'success-note' : ''}`}><span className="contract-glyph">{isMockDataSource() ? 'DEMO' : 'API'}</span><div><strong>{isMockDataSource() ? 'Laboratorio simulado' : 'Ejecución live'}</strong><p>{isMockDataSource() ? 'Las métricas son narrativas para demostrar el flujo; no constituyen evidencia de tesis.' : 'Cada repetición se ejecuta de verdad en RAG Core y en el Sandbox; puede tardar varios minutos.'}</p></div></div></div>{startMutation.isError && <div className="structured-error" role="alert"><strong>No se pudo iniciar el experimento</strong><p>{startMutation.error.message}</p></div>}{experimentId && (!operation || operation.status !== 'COMPLETED') && <div className="experiment-progress" role="status"><div><span className="live-indicator" /><strong>{operation?.status === 'RUNNING' ? 'Ejecutando estrategias pareadas' : 'Preparando experimento'}</strong><p>Experiment <code>{experimentId}</code></p></div><span>{operation?.progress ?? 0}%</span><i><b style={{ width: `${operation?.progress ?? 0}%` }} /></i></div>}{operation?.status === 'COMPLETED' && operation.result && <><div className="flow-connector" aria-hidden="true"><span>RESULT</span><i /></div><ExperimentComparison result={operation.result} projectId={projectId} experimentId={experimentId ?? ''} /></>}</section>
}
