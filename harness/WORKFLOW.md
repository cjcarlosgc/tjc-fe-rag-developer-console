...
3. **Post-tarea:** PULL `before-review`, revisión independiente y ux-reviewer para UI, más contract-reviewer si afecta contrato; roles compatibles pueden revisar en paralelo. Por defecto, antes de declarar el WI terminado, el usuario revisa el corte: leader presenta diff, criterios de aceptación, pruebas y evidencia, y espera su veredicto. Solo si el usuario lo solicita explícitamente, leader delega la revisión técnica independiente a un agente `reviewer`. El rol `ux-reviewer` permanece como revisión especializada obligatoria para cambios UI. Leader hace fan-in, corrige hallazgos (máximo dos ciclos), PULL `before-done`, valida gates y registra el cierre. Después hay revisión acumulada del rango antes de cualquier push solicitado.

Las asignaciones pueden quedar vacías hasta que se designen sus responsables. Al entrar en `W-IN_REVIEW`, `implementationAgent` debe identificar al implementer y `reviewAgent` al revisor independiente: usa `human-reviewer` por defecto y `reviewer` si el usuario delegó explícitamente en el agente. El implementer nunca puede revisarse a sí mismo. Los cortes con UI siguen requiriendo el rol separado `ux-reviewer`; no lo sustituye la revisión técnica independiente. Una delegación de revisión no sustituye la aprobación humana de alcance/arquitectura exigida en `W-AWAITING_APPROVAL`.

El implementer no aprueba su propio corte. Registra el veredicto técnico humano como handoff con `agent: human-reviewer`; si el usuario delegó, registra el handoff del `reviewer`. El handoff tiene `status` de veredicto (`APPROVED|CHANGES_REQUESTED|BLOCKED|DECISION_REQUIRED`), `findings`, `blockers`, `filesAffected`, `evidence` y `recommendedNextStep`; `evidence` enlaza el reporte verificable en `harness/reports/`. El handoff separado de `ux-reviewer` sigue siendo obligatorio para UI; el veredicto no sustituye los gates.
