...
- **T-003 / SYSTEM-2.4 / INTEROP-2.4 (2026-09-21):** HU62 (`DEC-ORG-001`). La Console pasa a login solo con GitHub: se retiran correo y contraseña, recuperación y solicitud de acceso, y `linkGitHub`. `401 GITHUB_IDENTITY_REQUIRED` es terminal (sin bucle ni redirección automática), `503 IDENTITY_UNAVAILABLE` y `503 GITHUB_VERIFICATION_UNAVAILABLE` son reintentables sin cerrar sesión, y los `401` del discovery de GitHub llevan a «Renovar acceso a GitHub» en lugar de cerrar la sesión. El WebSocket envía el token del handshake y maneja `connect_error` con reintento acotado. Los espejos SYSTEM-2.4/INTEROP-2.4 se copian desde Core y se registran HU58-HU64 en el backlog; workspaces y roles esperan el sync del bundle B. Orden de despliegue: esta Console antes que el bundle A de Core.
